...
max_cores_s = 6
try:
    max_cores = int(os.environ.get("SLURM_CPUS_PER_TASK", os.cpu_count())) - 1
except TypeError:
    max_cores = 4
...
def compute_parameters_case(
    parameters_case_template, A_OBJ, save_dir, N_RUNS_A, sample_num, case_num=1, max_cores_s=6, max_cores_r=6
):
    t0 = time.time()
...
if __name__ == "__main__":
    parser=argparse.ArgumentParser(description="Run baseline evaluation with multiprocessing")
    ...
    parser.add_argument("--max_cores", type=int, default=6, 
                        help="Max cores to use (Applies to both optimization and SOBOL)")
    ...
    compute_parameters_case(
        ...
        args.sample_num,
        case_num=1,
        max_cores_s=args.max_cores,
        max_cores_r=args.max_cores,
    )
    # case 2 parameters
    compute_parameters_case(
        parameters_case_template,
