...
def compute_weighted_MSE(I_dict, A_VARS, A_OBJ):
    ...
    SP = {
        var_name: {"bounds": CURRENT_BOUNDS, "start": 0.5} for var_name in I_dict.keys()
    }
...
def compute_jacobian(I_dict, A_VARS, A_OBJ, delta=1e-5):
    ...
    return J, np.linalg.matrix_rank(J)
...
@time_computation
def compute_hessian_GN(I_dict, A_VARS, A_OBJ, delta=1e-5):
    """
    Gauss-Newton Hessian matrix computation
    """
    J = compute_jacobian(I_dict, A_VARS, A_OBJ, delta=1e-5)[0]
    weight_mat = np.diag([i[0]["weight"] for i in list(A_OBJ.values())])
    H = 2 * J.T @ weight_mat @ J
    return H
...
def total_scan_execution(A_VARS_test, A_OBJ_test, sample_num):
    ...
    print(
        f'Scan objectives from position: {["_".join(reversed(i[0]["measure"]))+"="+str(i[0]["goal"])+" @ position "+str(list(A_OBJ_test.keys())[j])+ " with weight "+str(i[0]["weight"]) for j, i in enumerate(list(A_OBJ_test.values()))]}'
    )
    t0 = time.time()
    output_list = list()
    time_log = dict()
    for i in range(sample_num):
        I_dict_test = {}
        for j in A_VARS_test.values():
            I_dict_test[j[0]] = np.random.uniform(0.01, 1.5)
        ...
        # compute jacobian
        J, rank_J, time_jac = compute_jacobian(I_dict_test, A_VARS_test, A_OBJ_test)
        # compute hessian
        H, time_hess = compute_hessian_phi(I_dict_test, A_VARS_test, A_OBJ_test)
        # compute Gauss-Newton hessian
        H_GN, time_hess_GN = compute_hessian_GN(I_dict_test, A_VARS_test, A_OBJ_test)
        ...
        output_list.append(
            {
                ...
                "parameters": {
                    ...
                    "Hessian": H,
                    "Hessian_GN": H_GN,
                    "distance_H_H_GN": np.linalg.norm(H - H_GN, ord=1),
                    "eigenvalues": eigvals,
...
def SOBOL_sim(current, sample_number, A_VARS, A_OBJ, threshold_1=0.1, threshold_2=0.5):
    ...
    """
    samples_close = get_random_samples(
        local_min=current,
    ...
    samples_far = get_random_samples(
        local_min=current,
        bounds=CURRENT_BOUNDS,
        num_samples=sample_number,
        thres_1=threshold_1,
    ...
    for sample in samples_close:
        ...
        # compute jacobian
        J, rank_J, time_jac = compute_jacobian(sample, A_VARS, A_OBJ)
        # compute hessian
        ...
        result_close.append(
            {
                ...
                "parameters": (phi, J, rank_J, H, eigvals, eigvecs),
                "time": (time_mse, time_jac, time_hess, time_eig),
            }
        )
...
def evaluate_single_sample(sample, A_VARS, A_OBJ):
    ...
    try:
        ...
        J, rank_J, time_jac = compute_jacobian(sample, A_VARS, A_OBJ)
        H, time_hess = compute_hessian_phi(sample, A_VARS, A_OBJ)
        H_GN, time_hess_GN = compute_hessian_GN(sample, A_VARS, A_OBJ)
        eigvals, eigvecs, time_eig = compute_eig(H)
        t1=time.time()
        return {
                "current": tuple(sample.values()),
        ...
        return {
            "error": str(e), 
            "current": sample, 
            "traceback": "Failed during computation"
        }


# --- parallelized SOBOL_sim ---
def SOBOL_sim_parallel(
    current,
    sample_number,
    A_VARS,
    A_OBJ,
