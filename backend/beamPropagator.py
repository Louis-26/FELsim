...
def propagate(beamline: List,
    ...
    for idx, segment in enumerate(beamline):
        ...
        while remaining - interval > EPS:
            ...
            current = torch.tensor(segment.useMatrice(current, length=interval))
        ...
        if remaining > EPS:
            ...
            current = torch.tensor(segment.useMatrice(current, length=remaining))
