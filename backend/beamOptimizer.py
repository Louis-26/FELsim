...
class beamOptimizer:
    def __init__(self, beamline, matrixVariables, use_log=False, log_epsilon=1e-13):
        ...
        self.OBJECTIVEMETHODS = {
            "std": ebeam.std,
            "epsilon": ebeam.epsilon,
            "alpha": ebeam.alpha,
            "beta": ebeam.beta,
            "gamma": ebeam.gamma,
            "phi": ebeam.phi,
            "envelope": ebeam.envelope,
            "dispersion": ebeam.disper,
        }
    ...
    def calc(
        ...
        for indice in self.segmentVar:
            if indice < 0 or indice >= len(self.beamline):
                raise IndexError(
                    str(indice) + " is out of bounds for segmentVar dictionary"
                )
        ...
        for key, value in self.objectives.items():
            if key not in range(len(self.beamline)):
                raise TypeError(
                    "Invalid indice: indice "
                    + str(key)
                    + " in objectives dict is out of bounds"
                )
            for goal in value:
                if goal["measure"][1] in self.OBJECTIVEMETHODS:
                    ...
                    raise TypeError(
                        "Invalid method name: No such method name exists in OBJECTIVESMETHOD dict"
                    )
        ...
        for var in startPoint:
            ...
            if "start" in startPoint.get(var):
        ...
        if plotProgress:
            ...
            plot_iterate_np = np.array(
                [i.item() if torch.is_tensor(i) else i for i in self.plotIterate]
            )
            plot_mse_np = np.array(
                [m.item() if torch.is_tensor(m) else m for m in self.plotMSE]
            )

            # plot MSE line
            (mseLine,) = ax[1].plot(
                plot_iterate_np, plot_mse_np, label="Mean Squared Error", color="black"
            )
            ...
            lin_thresh_val = (
                10 ** (np.ceil(np.log10(float(mini)))) if mini > 0 else 1e-6
            )
            ...
            tempTrackVari = np.array(
                [
                    [v.item() if torch.is_tensor(v) else v for v in row]
                    for row in self.trackVariables
                ]
            )
            ...
            for i in range(len(tempTrackVari[0])):
                (varLine,) = ax[0].plot(
                    plot_iterate_np,
                    tempTrackVari[:, i],
                    label=self.variablesToOptimize[i],
                )
            ...
            calc_time = (
                (endTime - startTime) / self.iterationTrack
                if self.iterationTrack > 0
                else 0
            )
            timeLine = mlines.Line2D(
                [], [], color="white", label=f"{round(calc_time, 4)} s/iteration"
            )
    ...
    def testFuncIt(self, iterations):
        ...
        for i in range(iterations):
            result = self.calc()
            try:
                iterationsTotal += result.nit
            except AttributeError:
                raise AttributeError("algorithm does not track iterations")
        avIterate = iterationsTotal / iterations
        return avIterate

    def _prepare(self, segmentVar, startPoint, objectives):
        """
        Initialize internal state (segmentVar, objectives, variablesToOptimize,
        bounds, tracking lists) without running optimization.
        Shared setup used by both calc() and evaluate().
        """
        #  Tracking lists
        self.plotMSE = []
        self.plotIterate = []
        ...
        for key, value in self.objectives.items():
            ...
            for goal in value:
                ...
                self.trackGoals.update(
                    {"indice " + str(key) + ": " + goal["measure"][0] + " " + goal["measure"][1].__name__: []}
                )
    ...
    def evaluate(self, variableVals, segmentVar, startPoint, objectives) -> float:
        """
        Public: compute the MSE objective at given variable values, WITHOUT optimizing.
        Useful for landscape scans / sensitivity analysis.

        Parameters
        ----------
        variableVals : array-like
            Values for the optimization variables (order matches variablesToOptimize).
        segmentVar, startPoint, objectives : same as calc()
            Define the problem (which segments vary, bounds, what objectives to measure).

        Returns
        -------
        float : MSE at variableVals
        """
        import numpy as np
        self._prepare(segmentVar, startPoint, objectives)
        return float(self._optiSpeed(np.asarray(variableVals, dtype=float)))